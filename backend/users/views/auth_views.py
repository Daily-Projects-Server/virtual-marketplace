...
class LoginView(APIView):
    ...
    def post(self, request, format=None):
        try:
            ...
            return Response({"detail": "An internal error has occurred. Please try again later."}, status=status.HTTP_400_BAD_REQUEST)
...
class RegisterView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            if 'email' in e.detail and any('unique' in str(error).lower() for error in e.detail['email']):
                return Response(
                    {"email": ["User with this email already exists."]},
